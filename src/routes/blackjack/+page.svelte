...
<div class="space-y-4 pb-24">
	<div>
		<h1 class="text-2xl font-black tracking-tight">Blackjack</h1>
	...
	<!-- Table -->
	<div class="rounded-2xl border border-emerald-900/60 bg-gradient-to-b from-emerald-950 to-slate-950 p-4 space-y-4 shadow-2xl">
		<!-- Dealer -->
		<div>
			...
			<div class="flex gap-1.5 min-h-[4.4rem] items-center">
				{#if table}
		...
		<!-- Player hands -->
		<div>
			<div class="text-[0.65rem] uppercase tracking-widest text-white/40 mb-1.5">
				You{#if table && table.hands.length > 1}&nbsp;· {table.hands.length} hands{/if}
			</div>
			<div class="space-y-2">
				{#each table?.hands ?? [] as hand, i}
					<div
						class="rounded-xl p-2 transition-all {hand.active
				...
				{:else}
					<div class="h-[4.4rem] grid place-items-center text-white/25 text-sm">No hand in play.</div>
				{/each}
			</div>
		</div>
		...
		<div class="space-y-2">
			<div class="flex gap-2">
				{#each activeHand.moves as move}
					<button
						class="btn flex-1 font-black {move === 'stand'
							? 'btn-error'
							: move === 'hit'
								? 'btn-primary'
	...
	{:else}
		...
		<div class="flex items-center gap-3">
			...
			<button
				class="btn btn-lg btn-primary flex-1 font-black text-lg shadow-xl shadow-primary/30"
